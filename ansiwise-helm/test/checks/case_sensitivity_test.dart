  ...
  group('counter-probe', () {
    ...
    test('an import whose case differs from the on-disk name is reported', () {
      final List<Finding> reported = _reportedIn(<String, String>{
        'lib/src/steps/helm_release.dart': 'const int x = 1;',
        'lib/wrong.dart': "import 'src/steps/Helm_release.dart';",
      });
      expect(reported, hasLength(1), reason: 'this scan cannot go red on the defect it exists for');
      expect(
        reported.single.what,
        contains('lib/src/steps/helm_release.dart'),
    ...
    test('an import that matches byte for byte is not reported', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/helm_release.dart': 'const int x = 1;',
          'lib/right.dart': "import 'src/steps/helm_release.dart';",
        }),
        isEmpty,
    ...
    test('a relative path that climbs is resolved before it is compared', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/helm_repository.dart': 'const int x = 1;',
          'lib/src/wrong.dart': "import 'steps/Helm_repository.dart';",
        }),
        hasLength(1),
      );
    });
    ...
    test('an export and a part are judged like an import', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/plugin.dart': 'const int x = 1;',
          'lib/exports.dart': "export 'src/Plugin.dart';",
          'lib/parent.dart': "part 'src/Plugin.dart';",
        }),
        hasLength(2),
