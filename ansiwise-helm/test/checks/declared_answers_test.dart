...
Future<void> main() async {
  ...
  final PlantedAnswers planted = await answersDeclaredBy(helmRegistry);
  final Set<String> read = <String>{
    for (final RegisteredStep entry in helmRegistry.steps.values) ...entry.answers,
  };
  ...
  test('no step of this package reaches for an answer, so nothing is planted from a declaration', () {
    // Said outright rather than left to the count above. Every step here takes what it needs as an
    // ARGUMENT, out of the program row that names it, and reads nothing out of the run. The day one
    // of them does, this goes red and the assertions its neighbours carry — the declared default,
    // the closed set of values — belong here too.
    expect(
      read,
      isEmpty,
      reason:
          'a step here has begun reading an answer; give this check what the other packages hold '
          'against their declarations, or its planting is measured by nothing',
    );
  });
