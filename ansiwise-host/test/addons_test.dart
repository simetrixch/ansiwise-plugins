...
/// The addons of the snap: reading which are on, switching them on and off, and waiting for them.
void main() {
  ...
  ///
  /// The snap's own words, and the whole reason nothing here reads an exit code: it prints this and
  /// EXITS ZERO. A fake shell that was told to answer with this text answers success too, which is
  /// exactly what the real one does.
  const String stopped = 'microk8s is not running, try microk8s start\n';
  ...
  group('switching the addons on', () {
    test('they go on in the order the row wrote them', () async {
      ...
      const EnableAddons step = EnableAddons(addons: <String>['rbac', 'dns', 'ingress']);
    ...
    test('an addon that is already on is not switched on again', () async {
      ...
      const EnableAddons step = EnableAddons(addons: <String>['rbac']);
    ...
    test('an addon is asked for with the arguments its row wrote', () async {
      ...
      const EnableAddons step = EnableAddons(addons: <String>['dns:185.12.64.1,185.12.64.2']);
    ...
    test('an addon asked for with arguments is satisfied once its name is on', () async {
      final HostMachine machine = HostMachine()
        ..shell.answers('microk8s status', status(on: <String>['dns'], off: <String>[]));

      const EnableAddons step = EnableAddons(addons: <String>['dns:185.12.64.1']);
      expect(await step.check(machine.contextFor(under)), isA<Satisfied>());
      expect(machine.changing, isEmpty);
    });
    ...
    test('the undo switches off the NAME and in reverse order', () async {
      final HostMachine machine = HostMachine()
        ..shell.answers('microk8s status', status(on: <String>[], off: <String>['rbac', 'dns']));

      const EnableAddons step = EnableAddons(addons: <String>['rbac', 'dns:185.12.64.1']);
      final StepContext context = machine.contextFor(under);
      final List<String> captured = await step.capture(context);
      await step.undo(context, captured);
      expect(machine.changing, <String>['microk8s disable dns', 'microk8s disable rbac']);
    });

    test('a node that cannot be read blocks rather than reporting the addons on', () async {
      final HostMachine machine = HostMachine()..shell.fails('microk8s status');
      const EnableAddons step = EnableAddons(addons: <String>['rbac']);
  ...
  group('waiting for them to show up', () {
    ...
    test('A STOPPED NODE DOES NOT SATISFY THIS WAIT, though its status exits zero', () async {
      // The refutation this step exists for. `microk8s status` on a node that is not running prints
      // that it is stopped and returns SUCCESS, so a wait built on the exit code — or on any wait
      // that takes a zero for a yes — is satisfied by a cluster that is down. What is measured here
      // is the OUTPUT: a stopped node prints no enabled section, so nothing is on and the wait goes
      // on waiting.
      final HostMachine machine = HostMachine()..shell.answers('microk8s status', stopped);

      final CommandResult answered = await machine.shell.run(addonStatusCommand);
      expect(
        answered.ok,
        isTrue,
        reason:
  ...
  group('switching the addons off', () {
    ...
    test('a row that names none is a step with nothing to do, not an error', () async {
      ...
      expect(await step.check(HostMachine().contextFor(under)), isA<Satisfied>());
    });
  });
}
