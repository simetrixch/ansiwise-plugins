...
final Map<String, Fixture> stepFixtures = <String, Fixture>{
  ...
  // postcondition holds: a fixture that flipped them together would report the step exercised on
  // a machine where one of its two commands did nothing.
  //
  // What `stat -c %f` prints is the mode as the machine stores it, in hex: the kind bits of a
  // directory over the permission bits. This step is handed the NAME of an account and reads the
  // numbers off the machine, so the fake has to carry an account at all. The name it is handed is
  // the probe's own plausible text, because the probe plants an answer under that name and fills
  // the step's `account_answer` with it.
