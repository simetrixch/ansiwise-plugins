  ...
  group('counter-probe', () {
    ...
    test('an import whose case differs from the on-disk name is reported', () {
      final List<Finding> reported = _reportedIn(<String, String>{
        'lib/src/steps/host/install_packages.dart': 'const int x = 1;',
        'lib/wrong.dart': "import 'src/steps/host/Install_packages.dart';",
      });
      expect(reported, hasLength(1), reason: 'this scan cannot go red on the defect it exists for');
      expect(
        reported.single.what,
        contains('lib/src/steps/host/install_packages.dart'),
    ...
    test('an import that matches byte for byte is not reported', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/host/install_packages.dart': 'const int x = 1;',
          'lib/right.dart': "import 'src/steps/host/install_packages.dart';",
        }),
        isEmpty,
    ...
    test('a relative path that climbs is resolved before it is compared', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/host/ipv4.dart': 'const int x = 1;',
          'lib/src/steps/host/wrong.dart': "import '../host/IPv4.dart';",
        }),
        hasLength(1),
      );
    });
    ...
    test('an export and a part are judged like an import', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/host/install_snap.dart': 'const int x = 1;',
          'lib/exports.dart': "export 'src/steps/host/Install_snap.dart';",
          'lib/parent.dart': "part 'src/steps/host/Install_snap.dart';",
        }),
        hasLength(2),
    ...
    test('a finding names the line the directive sits on', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/host/install_snap.dart': 'const int x = 1;',
          'lib/wrong.dart': "// a first line\nimport 'src/steps/host/Install_snap.dart';",
        }).single.line,
        2,
