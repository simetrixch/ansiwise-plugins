...
final class RequireCommands extends ObservingStep {
  ...
  // THIS STEP DOES NOT DECLARE THAT IT RESTS ON AN EARLIER ONE. It is the one gate genuinely used
  // both ways: after an install, to prove the install produced the commands; and at the HEAD of a
  // program, to refuse a machine that does not carry what everything below it needs.
  //
  // Declared on the class, the second use is deferred along with the first. A program whose FIRST
  // row demands a tool the machine does not have reports that row as fine and fails several steps
  // later with a message about a chart repository, where the truth is a missing command. That is a
