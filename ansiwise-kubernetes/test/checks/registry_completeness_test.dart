...
void main() {
  ...
  final RegistryReading reading = RegistryReading.of(kubernetesRegistry);
  ...
  group('counter-probe', () {
    ...
    final SourceTree planted = SourceTree.planted(<String, String>{
      ...
      _plantedPath: _plantedDeclarations,
    });

    test('the planted declaration on line 4 is recognised', () {
      expect(
        declaresClass(planted, path: _plantedPath, line: 4, className: 'PlantedStep'),
        isTrue,
    ...
    test('line 3 only names the class in a comment and is not its declaration', () {
      expect(
        declaresClass(planted, path: _plantedPath, line: 3, className: 'PlantedStep'),
        isFalse,
      );
    });
    ...
    test('line 5 is not the declaration, so a drifted line number does not pass', () {
      expect(
        declaresClass(planted, path: _plantedPath, line: 5, className: 'PlantedStep'),
        isFalse,
      );
    });
    ...
    test('the class name on the line is read, not just the shape of the line', () {
      expect(
        declaresClass(planted, path: _plantedPath, line: 4, className: 'PlantedHelper'),
        isFalse,
      );
    });
    ...
    test('an ordinary repository-relative path is accepted', () {
      expect(
        sourceIsRepositoryRelative(_plantedPath),
        isTrue,
    ...
    test('a step class with no entry is reported', () {
      ...
      expect(
        about(onPlanted.findings, _plantedPath),
        isNotEmpty,
    ...
    test('an entry whose source points at the wrong line is reported', () {
      final RegistryCompleteness drifted = RegistryCompleteness(
        tree: planted,
        reading: RegistryReading.of(_registryOf(_entryAt('$_plantedPath:5'))),
      );
      expect(
    ...
    test('the same entry pointing at the declaration is left alone', () {
      final RegistryCompleteness onTarget = RegistryCompleteness(
        tree: planted,
        reading: RegistryReading.of(_registryOf(_entryAt('$_plantedPath:4'))),
      );
      expect(
    ...
    // The predicate half. This package registers no predicate, so nothing in its own registry
    // exercises these lines — which is exactly why they are planted here: the day this plugin
    // registers one, the rule that judges it has been shown to work rather than assumed to.
    final SourceTree plantedPredicateTree = SourceTree.planted(<String, String>{
      'pubspec.yaml': 'name: planted_package\n',
      _plantedPredicatePath: _plantedPredicateDeclaration,
    });

    test('a predicate whose source points at the wrong line is reported', () {
      final RegistryCompleteness drifted = RegistryCompleteness(
        tree: plantedPredicateTree,
        reading: RegistryReading.of(_registryOfPredicate('$_plantedPredicatePath:1')),
      );
      expect(
    ...
    test('a predicate pointing at its declaration is left alone', () {
      final RegistryCompleteness onTarget = RegistryCompleteness(
        tree: plantedPredicateTree,
        reading: RegistryReading.of(_registryOfPredicate('$_plantedPredicatePath:2')),
      );
      expect(
    ...
    test('a predicate that describes nothing is reported', () {
      final RegistryCompleteness silent = RegistryCompleteness(
        ...
        reading: RegistryReading.of(
          _registryOfPredicate('$_plantedPredicatePath:2', describes: '  '),
        ),
      );
      expect(
...
/// Where the planted step file sits in the tree this check writes.
const String _plantedPath = '$stepsDirectory/planted.dart';

/// Where the planted predicate file sits in the tree this check writes.
const String _plantedPredicatePath = '$stepsDirectory/planted_predicate.dart';
...
final String _plantedDeclarations = <String>[
  ...
  '}',
  '',
  // The four shapes a real step wears. A probe that plants only the first one proves the scan works
  // on a step with no type argument and says nothing about the rest, which is how a scan comes to
  // look past half the classes on disk while staying green.
