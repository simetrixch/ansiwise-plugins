  ...
  group('counter-probe', () {
    ...
    test('an import whose case differs from the on-disk name is reported', () {
      final List<Finding> reported = _reportedIn(<String, String>{
        'lib/src/steps/kubernetes_object.dart': 'const int x = 1;',
        'lib/wrong.dart': "import 'src/steps/Kubernetes_object.dart';",
      });
      expect(reported, hasLength(1), reason: 'this scan cannot go red on the defect it exists for');
      expect(
        reported.single.what,
        contains('lib/src/steps/kubernetes_object.dart'),
    ...
    test('an import that matches byte for byte is not reported', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/kubernetes_object.dart': 'const int x = 1;',
          'lib/right.dart': "import 'src/steps/kubernetes_object.dart';",
        }),
        isEmpty,
    ...
    test('a relative path that climbs is resolved before it is compared', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/kubectl.dart': 'const int x = 1;',
          'lib/src/wrong.dart': "import 'steps/Kubectl.dart';",
        }),
        hasLength(1),
      );
    });
    ...
    test('an export and a part are judged like an import', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/kubernetes_namespace.dart': 'const int x = 1;',
          'lib/exports.dart': "export 'src/steps/Kubernetes_namespace.dart';",
          'lib/parent.dart': "part 'src/steps/Kubernetes_namespace.dart';",
        }),
        hasLength(2),
    ...
    test('a finding names the line the directive sits on', () {
      expect(
        _reportedIn(<String, String>{
          'lib/src/steps/kubernetes_namespace.dart': 'const int x = 1;',
          'lib/wrong.dart': "// a first line\nimport 'src/steps/Kubernetes_namespace.dart';",
        }).single.line,
        2,
