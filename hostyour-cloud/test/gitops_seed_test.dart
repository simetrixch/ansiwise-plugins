...
/// Filling the secret store: the shape every workload authenticates against, and the values behind
/// it.
///
...
/// nothing at all, or every re-run of the installer rotates values that running pods read once.
void main() {
  ...
  /// The profile as the branch stamp writes it, which is where every step reads these three.
  ...
  group("the names a program file cannot write down", () {
    ...
    /// The profile as the branch stamp writes it, naming [name] and [mount].
    ...
    test('a policy name follows the short name and never the mount path', () async {
      // The two agree on every installation there is — the stamp writes the mount as
    ...
    test('an auth mount follows its own key and is never composed from the short name', () async {
      ...
      expect(
        ...
        reason:
            'the charts that render a secret store read this same key to decide where to log in, '
            'so a mount created anywhere else is a mount nothing on the cluster reaches',
      );
    });
    ...
    test('a profile that carries no short name blocks, by file and by key', () async {
      ...
      expect(result.reason, contains(clusterNameKey));
      expect(
        http.sent,
        isEmpty,
    ...
    test("a role name and an entry path take THIS run's stage", () async {
      // Under prod. A stage read from anywhere but the run would bind a prod installation's tenants
      // to the role a dev installation's readers log in under, and seed the dev tree.
      ...
      final StepPlan role = await const VaultAuthRole(
        ...
        role: 'tenant-eso-$stagePlaceholder',
        body: '{"token_policies":["$clusterPlaceholder-tenant-read"]}',
      ).plan(prod.context);
      expect(role.summary, endsWith('/v1/auth/$authMount/role/tenant-eso-prod'));
      expect((role as RequestPlan).body, contains('"m1-tenant-read"'));
    ...
    test('the browser role is sent back to the Vault the profile names', () async {
      ...
      // installation's Vault, and a slave's Vault is the master's — which only the profile records.
      final ({StepContext context, MemoryRecorder recorder}) it = contextOf(
        files: machineNaming(clusterName, authMount),
        http: ScriptedHttp((HttpRequest request, int nth) => answer('', status: 404)),
        step: 'vault_auth_role',
  ...
  group('the policies', () {
    test('a templated policy is written with the accessor the store minted', () async {
      ...
      const String rules =
          'path "secret/data/$stagePlaceholder/consumer/'
      ...
      const VaultPolicy step = VaultPolicy(
        repository: repository,
        name: '$clusterPlaceholder-consumer-read',
        rules: rules,
    ...
    test('a templated policy with no mount to template on is refused', () async {
      ...
      final CheckResult result = await const VaultPolicy(
        repository: repository,
        name: '$clusterPlaceholder-tenant-read',
        rules:
            'path "secret/data/$stagePlaceholder/tenants/'
            '{{identity.entity.aliases.$accessorPlaceholder.metadata.tenant}}" '
            '{ capabilities = ["read"] }',
        authMount: kubernetesMountPlaceholder,
      ).check(it.context);
  ...
  group('the roles', () {
    ...
    test('a role write keeps the namespaces something else added to it', () async {
      ...
      // the namespace of every cluster onboarded after that file was written, and their readers are
      // then refused with "not authorized" and nothing naming why.
      final ScriptedHttp http = ScriptedHttp((HttpRequest request, int nth) {
