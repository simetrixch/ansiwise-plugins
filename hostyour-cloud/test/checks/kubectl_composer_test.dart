  ...
  group('counter-probe', () {
    ...
    final List<Finding> planted = _spelledInvocations(
      SourceTree.planted(<String, String>{
        ...
        'lib/src/steps/host/planted.dart': _spellsTheExecutable,
        'lib/src/steps/host/planted_word.dart': _spellsTheWordInAnArgvList,
        'lib/src/steps/kubectl.dart': _spellsTheExecutable,
        'lib/src/steps/host/only_says_it.dart': _mentionsItInProse,
      }),
    );

    for (final String path in <String>[
      'lib/src/steps/host/planted.dart',
      'lib/src/steps/host/planted_word.dart',
    ]) {
    ...
    test('prose that names the client without quoting it is not reported', () {
      expect(about(planted, 'lib/src/steps/host/only_says_it.dart'), isEmpty);
    });

    test('a finding names the line the invocation sits on', () {
      expect(
        about(planted, 'lib/src/steps/host/planted.dart').map((Finding hit) => hit.line),
        contains(2),
...
const String _spellsTheWordInAnArgvList =
    "const List<String> plantedArgv = <String>['microk8s', 'kubectl', 'get', 'nodes'];";
