...
/// Refuses a machine that is not running the operating system release this platform is pinned to.
///
/// **The release is a pin like any other version, not a preference.** Everything above it was
/// proven on exactly this release: the microk8s channel we pin was, and a different release fails
/// at the snap — or worse, silently at the kernel, which is where the MongoDB incident recorded in
/// `platform/versions.yaml` came from.
///
...
final class RequirePinnedUbuntu extends ObservingStep {
  ...
  static const List<ArgumentSpec> arguments = <ArgumentSpec>[
    ArgumentSpec(
      ...
      describes: 'the operating system release every machine of this platform runs, as VERSION_ID',
    ),
  ];
  ...
  Future<CheckResult> check(StepContext context) async {
    if (!await context.files.exists(_osRelease)) {
      return const CheckResult.blocked(
        '/etc/os-release is not there, so this is not a machine this platform runs on',
      );
    }
    ...
    if (id != 'ubuntu') {
      return CheckResult.blocked(
        ...
        'and the platform is pinned to ubuntu $release',
      );
    }
    if (version != release) {
      return CheckResult.blocked(
        'this machine runs ubuntu ${version ?? 'an unnamed release'}, '
        'and the platform is pinned to $release',
      );
    }
