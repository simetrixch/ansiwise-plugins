...
final class RequireCommands extends ObservingStep {
  ...
  ///
  /// `htpasswd` is on this list for a reason worth keeping: bcrypt is the only hash the registry and
  /// the ingress basic-auth accept, and `openssl` cannot produce it. There is no fallback recipe, so
  /// the tool has to be on the machine before any password is written.
