...
///
/// **What the overlap costs, measured on a live cluster.** MicroK8s ships Calico with the default
/// pool `10.1.0.0/16`, and a typical cluster LAN of `10.1.1.0/24` sits inside it. Pod traffic to that
/// LAN is then routed into the overlay, or leaves without being translated and the reply is
/// swallowed by the peer's own overlapping pool. Four things on a master dial another cluster's API
/// server and all four time out together: that cluster's deployment instance, the secret store on
/// every login it performs, the shared dashboard's credentials, and the controller's own client —
/// which is the one that writes.
///
/// **Two ranges must be avoided and only one of them is configurable.** The service range is fixed
/// by MicroK8s at `10.152.183.0/24`, and the LAN is whatever this machine shares with the other
/// clusters. Leaving the LAN unset skips its half of this gate, which is a choice and not an
...
final class RequirePodCidrFreeOfReservedRanges extends ObservingStep {
  ...
  static const List<ArgumentSpec> arguments = <ArgumentSpec>[
    ...
    ArgumentSpec(
      name: 'service_cidr',
      kind: ArgumentKind.text,
      describes: 'the fixed range MicroK8s hands cluster IPs out of, which nothing may overlap',
      required: false,
      defaultValue: '10.152.183.0/24',
    ),
  ];
  ...
  /// The fixed range MicroK8s hands cluster IPs out of.
  ...
  Future<CheckResult> check(StepContext context) async {
    ...
    if (problems.isEmpty) {
      if (overlap(podCidr, serviceCidr)) {
        problems.add(
          '$podCidr overlaps $serviceCidr, which is the range MicroK8s hands cluster IPs out of and '
          'is not configurable',
        );
      }
