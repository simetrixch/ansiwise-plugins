...
///
/// **The content may carry one marked slot, and only one.** A program file ships inside the binary
/// to every installation, so a value belonging to ONE machine cannot stand in it — see
/// [DetectHostUpstreamResolvers.placeholder]. Anything else between angle brackets is content and is
/// written as it stands, because a ConfigMap key legitimately holds markup.
///
...
final class PatchConfigmapKey extends ReversibleStep<String?> {
  ...
  static const List<ArgumentSpec> arguments = <ArgumentSpec>[
    ...
    ArgumentSpec(
      ...
      describes:
          ...
          'carry "${DetectHostUpstreamResolvers.placeholder}" where the name servers this machine '
          'reaches the internet through belong',
    ),
    ArgumentSpec(
      name: 'rollout',
      kind: ArgumentKind.text,
  ...
  Future<CheckResult> check(StepContext context) async {
    ...
    final String? wanted = await DetectHostUpstreamResolvers.filled(context, content);
    if (wanted == null) {
      return CheckResult.blocked(
        'the content of $key carries ${DetectHostUpstreamResolvers.placeholder} and this machine '
        'names no name server a pod could reach, so there is nothing to write there',
      );
    }
    if (live == wanted) {
  ...
  Future<StepPlan> plan(StepContext context) async => StepPlan.diff(
    ...
    after: await DetectHostUpstreamResolvers.filled(context, content) ?? '',
  );
  ...
  Future<void> apply(StepContext context) async {
    ...
    if (wanted == null) {
      throw StateError(
        'the content of $key carries ${DetectHostUpstreamResolvers.placeholder} and this machine '
        'names no name server a pod could reach',
      );
    }
    final String? live = await _live(context);
    if (live != null) {
