...
/// this plugin ended up with steps running a plain `kubectl` beside steps running the one the
/// MicroK8s snap ships.
///
/// **The invocation is an argument, and the default is this platform's answer.** That a machine
/// reaches its cluster through `microk8s kubectl` is a fact about this platform, not about kubectl,
/// so it stands in [argument]'s default where a program row can read it and replace it. A cluster
/// reached through a plain `kubectl` on the path is one word in a row, not another edit to twenty
/// files.
///
...
final class Kubectl {
  ...
  const Kubectl([this.invocation = _microk8s]);
  ...
  /// The client the MicroK8s snap ships, which is the one this platform uses.
  static const List<String> _microk8s = <String>['microk8s', 'kubectl'];
  ...
  static const ArgumentSpec argument = ArgumentSpec(
    ...
    required: false,
    defaultValue: _microk8s,
    describes:
        'the words kubectl is invoked with, in front of every subcommand — the default is the '
        'client the MicroK8s snap ships, and a cluster reached through a plain kubectl on the '
        'path names that one word instead',
  );
