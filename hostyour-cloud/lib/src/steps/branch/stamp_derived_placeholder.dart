...
final class StampDerivedPlaceholder extends ReversibleStep<List<String>> {
  /// Replaces [placeholder] with this run's own branch everywhere in [repository] that holds it.
  const StampDerivedPlaceholder({
  ...
  factory StampDerivedPlaceholder.fromArguments(Arguments arguments) =>
      StampDerivedPlaceholder(
        repository: arguments.text('repository'),
        trunk: arguments.text('trunk'),
        placeholder: arguments.text('placeholder'),
        derivedValue: arguments.text('derived_value'),
  ...
  /// declaring it.
  static const List<String> answers = <String>['fqdn', 'role', 'master'];
  ...
  String? _replacement(StepContext context) {
    ...
    return switch (derivedValue) {
      'cluster' => fqdn.split('.').first,
      'cluster_name' => fqdn,
      'books_branch' => MasterPart.of(given).holdsMaster ? fqdn : given.text('master'),
      'master_name' => MasterPart.of(given).holdsMaster ? fqdn : given.text('master'),
      _ => throw StateError('unknown derived value: $derivedValue'),
    };
  }

  /// What is said about a run that answers nothing under the name this row named.
  String get _unanswered =>
      'this run failed to compute the derived value "$derivedValue" for "$placeholder"';
