/// The two files an operator's answers become, and the rules that decide what may be written into
/// them.
///
/// `configs/config.<stage>` and `secrets/secrets.<stage>` are the two files the rest of
/// this installation reads its own inputs out of. Neither is carried on the trunk: the trunk carries
/// `config.example` and `secrets.example`, which are the same files with every value at a
/// placeholder, and a run turns one into the other by filling the keys it owns and copying every
...
///    filled while the file kept nothing at all. [FilledTemplate.missingKeys] answers that question
...
///    template itself. See [FilledTemplate.isUnset].
...
final class FilledTemplate {
  ...
  const FilledTemplate({required this.template, required this.current});
