...
///
/// The four things this program puts on top of a standing cluster — the identity provider, the
/// dashboard, ArgoCD and Vault — are helm releases that ArgoCD itself does not manage, because
/// ArgoCD authenticates through the identity provider and a self-managing loop makes a broken one
/// unrecoverable. Nothing repairs these but a re-run of this.
///
/// **The version is pinned and there is no path that resolves the newest.** An unpinned upgrade asks
/// the repository what the latest is on every run, and for exactly these four that answer can cross
/// a major version by itself — on the components no GitOps reconciler is watching.
///
...
final class HelmRelease extends IrreversibleStep {
  ...
  String get irreversibleReason =>
      ...
      'sets bound stay behind holding what the release wrote — Vault\'s storage, the identity '
      'provider\'s database — and a later install lands on top of that rather than on an empty '
      'cluster';
