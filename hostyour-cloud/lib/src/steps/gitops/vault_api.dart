/// Reaching Vault over its HTTP API, and reading the files this stage's secrets stand in.
///
/// **Why the API and not a command inside the pod.** The shell this replaces talked to Vault by
/// running the `vault` binary inside `vault-0` through `kubectl exec`, and most of what its comments
/// are about follows from that one choice: `kubectl exec` arguments are visible in a process listing
/// to every process on the host, so the root token, the unseal keys, the OIDC client secret and the
/// token-reviewer JWT all had to be fed on standard input instead, and the role body had to be sent
/// as one JSON object because the container's own shell collapsed backslash-escaped JSON into a
...
///
/// The text is a contract rather than a formatting choice: the file is read back by this program on
/// every later run and by the host-side unsealer once a minute, and changing the label breaks both
/// at once with nothing naming the cause.
...
Future<RootToken> rootTokenFrom(StepContext context, String path) async {
  if (!await context.files.exists(path)) {
    return RootToken.unreadable(
      '$path is not on this host, and it is where Vault\'s root token is — the phase that '
...
String renderCredentials({
  ...
  '',
  'COPY THIS FILE OFF THE HOST. The unsealer on this node reads it to survive a restart, so it is',
  'also the only escrow left if this disk is lost. Re-initializing Vault is not a way to get another',
  'copy: it means destroying the storage and every secret in it.',
  '',
].join('\n');
...
/// every call answer 403, one inside an unseal key makes Vault reject that key once a minute
...
///
/// The one hand-filled input of the whole installation is written this way, and it is read into an
/// explicit map here rather than sourced. A shell sourced it into its own process so that indirect
/// expansion could reach the variables the schema names; there is no such thing to reproduce, and a
/// file that can only be read by running it is a file that can run anything.
...
///
/// An angle-bracketed name is how the example seed input says "fill this in", and no seeded value
