...
///
/// This is the only place in the whole platform where the root token and the unseal keys exist. If
...
final class VaultInit extends IrreversibleStep {
  ...
  /// `0600` — the file holds the keys to everything, and the host-side unsealer reads it as root.
  ...
  Future<void> apply(StepContext context) async {
    ...
    context.log.warn(
      'COPY $credentialsPath OFF THIS HOST. The unsealer reads it to survive a restart, so it is '
      'also the only escrow left if this disk is lost',
    );
  }
