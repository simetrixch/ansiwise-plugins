...
Future<void> main() async {
  ...
  final PlantedAnswers planted = await answersDeclaredBy(vaultRegistry);
  ...
  test('no step of this package reaches for an answer, so nothing is planted from a declaration', () {
    ...
    // ARGUMENT, out of the program row that names it, and what varies with the run is filled into
    // those values rather than read out of the run. The day a step reads an answer again, this goes
    // red and the assertions its neighbours carry — the declared default, the closed set of values
    // — belong here too.
    expect(
      read,
      isEmpty,
