...
/// deployment: which caller may read which path, and what may be written without being readable
/// back.
///
...
/// names the accessor in its path. The program file writes [accessorPlaceholder], this reads the
/// real one from Vault and hands it to the filling. Getting that wrong has no error: every templated
/// path resolves empty and the caller is refused with a message about its own token.
///
...
final class VaultPolicy extends ReversibleStep<String?> {
  ...
  _Names _namesIn(StepContext context, VaultProfile profile) {
