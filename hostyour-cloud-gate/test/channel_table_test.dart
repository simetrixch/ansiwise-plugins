...
void main() {
  ...
  group('the repository', () {
    ...
    test('reads chart material only, so a table outside it is no statement', () {
      // Planted, where this used to name ITSELF. The gate lived inside the audited repository, this
      // source carries the table four times as fixtures, and an audit that scanned every tracked
      // path would have reported them. The gate has moved out, so no source of it can stand in that
      // tree any more and the old guard could no longer fail — which is the one thing a check may
      // never become.
      //
      // What it protected is not about this file. It is that only the values chain decides a
      // deployment, so only a statement standing in the chain is a second table. A planted tree
      // asserts that directly, and it can still go red.
      final SourceTree statesItOutsideTheChain = SourceTree.planted(
        Directory('${scratch.path}${Platform.pathSeparator}outside')..createSync(recursive: true),
        const <String, String>{
          channelTableFile: _theDeclaration,
          'notes/decision-record.md': _aSecondTable,
          'apps/probe/build.sh': _aSecondTable,
        },
      );
      expect(
        ChannelTableAudit(
          statesItOutsideTheChain,
        ).statements().map((({String path, int line}) s) => s.path),
        <String>[channelTableFile],
        reason:
            'a table in a note or a script is not a table a cluster reads; an audit that scanned '
            'every tracked path would report both plants as second statements',
      );
    });
