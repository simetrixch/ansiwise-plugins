...
/// reports nothing.
///
/// The neighbours are the point. `lib/src/installation.dart` is a real file of this package, and a
/// check that collapsed back into a match on the substring would demand it be renamed to something
...
void main() {
  ...
  group('the repository', () {
    ...
    test('the neighbour the rule protects is a real file, here in this package', () {
      // `installation` begins with an abolished word and is not that word. That this is a REAL case
      // and not a hypothetical used to be asserted against the audited tree, because the gate stood
      // inside it. The gate has moved out, and the audited tree carries no such name at all — `git
      // ls-files | grep install` in it comes back empty — so the assertion is made where the file
      // now lives instead of being dropped.
      //
      // The guarantee is unchanged: a rule that collapsed back into a substring match would demand
      // this file be renamed to something that no longer says what it is.
      expect(
        File(p.join(Directory.current.path, 'lib', 'src', 'installation.dart')).existsSync(),
        isTrue,
        reason:
            'the neighbour the rule exists to protect; if it is renamed or gone, the counter-probe '
            'below is guarding a case nothing here has',
      );
    });
  });
