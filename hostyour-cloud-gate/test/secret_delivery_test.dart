...
void main() {
  ...
  group('the repository', () {
    ...
    test('judges chart material only, so a manifest outside it is not a document', () {
      // Planted, where this used to name ITSELF. The gate lived inside the audited repository, this
      // source carries three manifests off the rule as fixtures, and an audit that scanned every
      // tracked path would have reported them. The gate has moved out, so no source of it can stand
      // in that tree any more and the old guard could no longer fail — which is the one thing a
      // check may never become.
      //
      // The property is not about this file. It is that only what a cluster renders is judged, so a
      // manifest quoted in a note or pasted into a script is not one of this platform's
      // ExternalSecrets. Planted, it can still go red.
      final SourceTree quotesOneOutsideTheChain = SourceTree.planted(
        Directory('${scratch.path}${Platform.pathSeparator}outside')..createSync(recursive: true),
        const <String, String>{'notes/incident.md': _silent, 'apps/probe/apply.sh': _silent},
      );
      expect(
        SecretDeliveryAudit(
          quotesOneOutsideTheChain,
        ).documents().map((ExternalSecretDocument d) => d.path),
        isEmpty,
        reason:
            'a manifest quoted in a note or a script is not one a cluster reconciles; an audit that '
            'scanned every tracked path would report both plants and their defects',
      );
    });
