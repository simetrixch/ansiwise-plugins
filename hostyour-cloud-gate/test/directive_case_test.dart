...
/// Every Dart directive of THIS PACKAGE spells the path it names byte for byte.
///
/// It is edited on Windows and cloned onto Linux, and an import spelled `Tree/` for a directory
/// tracked as `tree/` is the one defect that is invisible on the first machine and fatal on the
/// second. The audit is [DirectiveCaseAudit]; this file drives it over this package, where its
/// findings must be empty, and over trees planted for the purpose.
///
/// WHY THIS PACKAGE AND NOT THE TREE UNDER AUDIT. The audited repository is charts and manifests and
/// ships no Dart, so a directive check pointed at it judges nothing and can never go red. That was
/// already true before the gate moved out of it — the gate was the only Dart in there, so this check
/// only ever judged the checker. The move makes the subject explicit instead of leaving it implied.
///
...
void main() {
  group('this package', () {
    final DirectiveCaseAudit audit = DirectiveCaseAudit(thisPackageTree());

    test('has directives of its own to judge', () {
      expect(
        audit.judged,
        isNotEmpty,
        reason: 'no directive here names a file of this package, so this check measured nothing',
      );
    });
